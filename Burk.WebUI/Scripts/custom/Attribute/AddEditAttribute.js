        ...
        $.ajax({
            ...
            success: function (result) {
                if (result == "Error") {
                    ...
                    UnblockUI();
                    panel.EditAttribute(result.slice(4, 44), $('#FullName').val(), $("#IsShowInGrid").is(":checked"), $("#IsReq").is(":checked"));
                    $('#AddEditDiv').dialog('destroy').html("");
                }
                else {
                    UnblockUI();
                    panel.AddAttribute($('#AttributeTypeName').val(), result, $('#FullName').val(), 200, 75, 0, 0, $("#IsShowInGrid").is(":checked"), $("#IsReq").is(":checked"), true, $('#AttributeTypeIndex').val());
                    $('#AddEditDiv').dialog('destroy').html("");
                }
            }
        });
